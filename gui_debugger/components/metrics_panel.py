...
class MetricsPanel(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        refresh_btn = ttk.Button(btn_frame, text="🔄 Обновить", command=self._refresh_metrics)
        refresh_btn.pack(side=tk.LEFT, padx=5)
        
        export_btn = ttk.Button(btn_frame, text="📤 Экспорт", command=self._export_metrics)
    ...
    def _create_stat_card(self, parent: tk.Widget, title: str, value: str, row: int, col: int) -> ttk.Frame:
        ...
        title_label = ttk.Label(card, text=title, font=("Segoe UI", 9), foreground="#606060")
        title_label.pack(anchor=tk.W)
        
        value_label = ttk.Label(card, text=value, font=("Segoe UI", 16, "bold"), foreground="#303030")
    ...
    def _refresh_metrics(self) -> None:
        ...
        try:
            ...
            self.cache_card.winfo_children()[1].configure(text=f"{cache_hits} ({cache_rate:.1f}%)")
