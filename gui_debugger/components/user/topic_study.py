...
class TopicStudy(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        # Настройка тегов
        self.answer_area.tag_configure("title", font=("Segoe UI", 12, "bold"), foreground="#0078d4")
        self.answer_area.tag_configure("text", font=("Segoe UI", 10), foreground="#ffffff")
        self.answer_area.tag_configure("meta", font=("Segoe UI", 9), foreground="#808080")
        
        # Обратная связь
        feedback_frame = ttk.Frame(self)
        feedback_frame.pack(fill=tk.X, padx=20, pady=10)
        
        ttk.Label(feedback_frame, text="Оцените ответ:", font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=10)
    ...
    def _show_welcome(self) -> None:
        ...
        self.answer_area.insert(tk.END, "Выберите тему из списка выше, чтобы получить подробное объяснение.\n\n", "text")
    ...
    def _select_topic(self, topic: str) -> None:
        ...
        self.status_label.configure(text=f"⏳ Запрос по теме: {topic}", foreground="#ffb900")
        self.answer_area.delete(1.0, tk.END)
        self.answer_area.insert(tk.END, f"Загрузка материала по теме: {topic}...\n", "meta")
    ...
    async def _async_query(self, query: str) -> dict:
        ...
        if self.rag_pipeline:
            return await self.rag_pipeline.get_answer(query, user_id=999999, use_cache=True)
        else:
            return {
    ...
    def _process_topic(self, topic: str) -> None:
        ...
        try:
            ...
            self.status_label.configure(text=f"✅ Тема изучена: {topic}", foreground="#107c10")
    ...
    def _give_feedback(self, feedback: str) -> None:
        ...
        """
        if feedback == "good":
            self.status_label.configure(text="✅ Спасибо за оценку!", foreground="#107c10")
        else:
            self.status_label.configure(text="⚠️ Попробуйте переформулировать вопрос", foreground="#ffb900")
