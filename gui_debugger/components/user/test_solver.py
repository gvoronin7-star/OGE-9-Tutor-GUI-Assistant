...
from gui_debugger.utils.gui_logger import log_action, log_error, gui_action_logger
...
class TestSolver(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        # Режим 1: По теме
        ttk.Label(mode_frame, text="По теме:", font=("Segoe UI", 10, "bold")).grid(row=0, column=0, padx=10, pady=5)
        
        # Выбор темы
        ttk.Label(mode_frame, text="Тема:", font=("Segoe UI", 9)).grid(row=1, column=0, padx=5, pady=5)
        self.topic_var = tk.StringVar(value=self.TOPICS[0])
        topic_combo = ttk.Combobox(mode_frame, textvariable=self.topic_var, values=self.TOPICS, state="readonly", width=25)
        topic_combo.grid(row=1, column=1, padx=5, pady=5)
        topic_combo.bind("<<ComboboxSelected>>", lambda e: self._update_available_count())
        ...
        # Разделитель
        ttk.Separator(mode_frame, orient=tk.VERTICAL).grid(row=1, column=4, rowspan=2, padx=20)
        
        # Режим 2: Все темы
        ttk.Label(mode_frame, text="Все темы:", font=("Segoe UI", 10, "bold")).grid(row=0, column=5, padx=10, pady=5)
        ...
        ttk.Label(mode_frame, text="Количество:", font=("Segoe UI", 9)).grid(row=2, column=5, padx=10, pady=5)
    ...
    def _update_available_count(self) -> None:
        ...
        if total_questions > 0:
            self.available_count_label.configure(
                text=f"📊 Доступно: {total_questions} вопросов",
                foreground="#107c10"
            )
        else:
            self.available_count_label.configure(
                text="📊 Доступно: 0 вопросов",
                foreground="#e81123"
            )
    ...
    def _start_all_topics_test(self) -> None:
        ...
        self.status_label.configure(text=f"⏳ Генерация теста по всем темам ({num_questions} вопросов)...")
    ...
    def _generate_test_async(self, topic: str) -> None:
        ...
        try:
            ...
            if test and "error" not in test:
                ...
                self.loading_label.configure(text="")
                messagebox.showerror("Ошибка", test.get("error", "Не удалось сгенерировать тест"))
                self._use_demo_test(topic)
        ...
        demo_test = {
            ...
            "questions": {
                ...
                "q_2": {
                    ...
                    "answers": ["Утверждение А", "Утверждение Б", "Утверждение В", "Утверждение Г"],
                    "correct_answer": 2,
    ...
    def _shuffle_answers(self, question: Dict) -> Dict:
        ...
        new_correct_idx = next(i for i, (_, is_correct) in enumerate(answer_list) if is_correct)
    ...
    def _use_demo_all_topics_test(self, num_questions: int = 10) -> None:
        ...
        demo_test = {
            ...
            "questions": {
                ...
                "q_4": {
                    ...
                    "answers": ["Мера стоимости, средство платежа", "Только обмен", "Только накопление", "Только платёж"],
                    "correct_answer": 0,
                ...
                "q_9": {
                    ...
                    "answers": ["Отсутствие работы у трудоспособных", "Отсутствие работы у всех", "Декрет", "Учеба"],
                    "correct_answer": 0,
        ...
        for q_key in demo_test["questions"]:
            demo_test["questions"][q_key] = self._shuffle_answers(demo_test["questions"][q_key])
    ...
    def _load_test(self, test: Dict[str, Any]) -> None:
        ...
        self.result_frame.pack_forget()
        self.loading_label.configure(text="")
        self.status_label.configure(text=f"✅ Тест загружен: {test['total_questions']} вопросов")
    ...
    def _select_answer(self, answer_index: int) -> None:
        ...
        # Логирование ответа
        question_time = time.time() - self.start_time if hasattr(self, 'start_time') else 0
    ...
    def _finish_test(self) -> None:
        ...
        self.result_frame.pack(fill=tk.X, padx=20, pady=10)
        self.result_label.configure(text=f"{emoji} Результат: {self.score}/{total} ({percentage:.0f}%) - {grade}")
        ...
        if self.on_test_complete:
            self.on_test_complete(
                self.current_test.get("topic", ""),
                self.score,
                total
            )
        
        # Логирование завершения теста
        log_action("test_completed", {
            "topic": self.current_test.get("topic", ""),
