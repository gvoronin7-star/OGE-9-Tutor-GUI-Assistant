...
class UsersPanel(ttk.Frame):
    ...
    def _get_demo_users(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"user_id": "12345", "active": True, "tests": 15, "accuracy": 85, "last_seen": "09.04 14:30"},
            {"user_id": "67890", "active": True, "tests": 8, "accuracy": 72, "last_seen": "09.04 13:15"},
            {"user_id": "11111", "active": False, "tests": 25, "accuracy": 91, "last_seen": "08.04 18:45"},
            {"user_id": "22222", "active": True, "tests": 3, "accuracy": 60, "last_seen": "09.04 12:00"},
            {"user_id": "33333", "active": True, "tests": 42, "accuracy": 88, "last_seen": "09.04 14:25"},
        ]
    ...
    def _create_widgets(self) -> None:
        ...
        active_check = ttk.Checkbutton(filters_frame, text="Только активные", variable=self.show_active_var, command=self._filter_users)
        ...
        details_btn = ttk.Button(actions_frame, text="📊 Детали", command=self._show_details)
        details_btn.pack(side=tk.LEFT, padx=5)
        
        block_btn = ttk.Button(actions_frame, text="🚫 Блокировать", command=self._block_user)
        block_btn.pack(side=tk.LEFT, padx=5)
        
        message_btn = ttk.Button(actions_frame, text="✉️ Сообщение", command=self._send_message)
        message_btn.pack(side=tk.LEFT, padx=5)
        
        export_btn = ttk.Button(actions_frame, text="📤 Экспорт", command=self._export_users)
        export_btn.pack(side=tk.RIGHT, padx=5)
        
        refresh_btn = ttk.Button(actions_frame, text="🔄 Обновить", command=self._refresh_users)
    ...
    def _block_user(self) -> None:
        ...
        if messagebox.askyesno("Подтверждение", f"Вы уверены, что хотите {action} пользователя {self.selected_user['user_id']}?"):
    ...
    def _send_message(self) -> None:
        ...
        def send():
            ...
            if message:
                messagebox.showinfo("Отправлено", f"Сообщение отправлено пользователю {self.selected_user['user_id']}")
                dialog.destroy()
            else:
    ...
    def _export_users(self) -> None:
        ...
        try:
            with open(file_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.DictWriter(f, fieldnames=["user_id", "active", "tests", "accuracy", "last_seen"])
                writer.writeheader()
                writer.writerows(self.users_data)
            
            messagebox.showinfo("Экспорт", f"Экспортировано {len(self.users_data)} пользователей")
