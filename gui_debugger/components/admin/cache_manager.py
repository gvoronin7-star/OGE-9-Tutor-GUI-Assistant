...
class CacheManager(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        """Создание виджетов."""
        # Заголовок
        header = ttk.Label(self, text="💾 УПРАВЛЕНИЕ КЭШЕМ", font=("Segoe UI", 14, "bold"))
        header.pack(pady=10)
        ...
        self.key_label = ttk.Label(details_frame, text="Ключ: —", font=("Segoe UI", 10, "bold"))
        ...
        self.size_label = ttk.Label(details_frame, text="Размер: —", font=("Segoe UI", 10))
        ...
        refresh_btn = ttk.Button(btn_frame, text="🔄 Обновить", command=self._refresh_stats)
        refresh_btn.pack(side=tk.LEFT, padx=5)
        
        clear_all_btn = ttk.Button(btn_frame, text="🗑️ Очистить всё", command=self._clear_all)
        clear_all_btn.pack(side=tk.LEFT, padx=5)
        
        clear_topic_btn = ttk.Button(btn_frame, text="🔍 По теме", command=self._clear_by_topic)
    ...
    def _create_stat_card(self, parent: tk.Widget, title: str, value: str, row: int, col: int) -> ttk.Frame:
    ...
    def _refresh_stats(self) -> None:
        ...
        try:
            ...
            self.keys_card.winfo_children()[1].configure(text=str(stats.get("keys_count", 0)))
            self.memory_card.winfo_children()[1].configure(text=f"{stats.get('used_memory', '0')} MB")
            self.hits_card.winfo_children()[1].configure(text=f"{stats.get('hit_rate', 0):.1f}%")
            self.clients_card.winfo_children()[1].configure(text=str(stats.get("connected_clients", 0)))
    ...
    def _clear_all(self) -> None:
        """Очистка всего кэша."""
        if messagebox.askyesno("Очистка кэша", "Вы уверены? Это удалит все закэшированные ответы."):
            try:
    ...
    def _clear_by_topic(self) -> None:
        ...
        def do_clear():
            ...
            if topic:
                ...
                messagebox.showinfo("Очистка", f"Найдено {count} ключей по теме '{topic}'")
                dialog.destroy()
    ...
    def _export_keys(self) -> None:
        ...
        try:
            ...
            messagebox.showinfo("Экспорт", f"Экспортировано {self.keys_listbox.size()} ключей")
