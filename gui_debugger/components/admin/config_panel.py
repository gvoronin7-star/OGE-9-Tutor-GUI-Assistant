...
class ConfigPanel(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        export_btn = ttk.Button(btn_frame, text="📤 Экспорт", command=self._export_config)
    ...
    def _create_rag_tab(self, parent: tk.Widget) -> ttk.Frame:
        ...
        # Top-K
        ttk.Label(tab, text="Top-K (количество чанков):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.top_k_var = tk.StringVar(value="5")
        top_k_spinbox = ttk.Spinbox(tab, from_=1, to=20, textvariable=self.top_k_var, width=10)
        top_k_spinbox.pack(anchor=tk.W, pady=5)
        
        # Threshold
        ttk.Label(tab, text="Threshold (порог схожести):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.threshold_var = tk.StringVar(value="0.5")
        threshold_spinbox = ttk.Spinbox(tab, from_=0.0, to=1.0, increment=0.1, textvariable=self.threshold_var, width=10)
        threshold_spinbox.pack(anchor=tk.W, pady=5)
        
        # Источники
        ttk.Label(tab, text="Приоритет источников:", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        
        sources_frame = ttk.Frame(tab)
        sources_frame.pack(anchor=tk.W, pady=5)
        
        self.rag_base_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(sources_frame, text="RAG_data_base (FIPI)", variable=self.rag_base_var).pack(side=tk.LEFT, padx=5)
        
        self.faiss_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(sources_frame, text="Local Faiss", variable=self.faiss_var).pack(side=tk.LEFT, padx=5)
        
        self.whoosh_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(sources_frame, text="Whoosh", variable=self.whoosh_var).pack(side=tk.LEFT, padx=5)
    ...
    def _create_llm_tab(self, parent: tk.Widget) -> ttk.Frame:
        ...
        # Модель
        ttk.Label(tab, text="Основная модель:", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.model_var = tk.StringVar(value="GigaChat-Max")
        model_combo = ttk.Combobox(tab, textvariable=self.model_var, values=[
            "GigaChat-Max",
            "GigaChat-Pro",
            "YandexGPT-Lite"
        ...
        # Timeout
        ttk.Label(tab, text="Timeout (секунды):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.timeout_var = tk.StringVar(value="30")
        timeout_spinbox = ttk.Spinbox(tab, from_=10, to=120, textvariable=self.timeout_var, width=10)
        timeout_spinbox.pack(anchor=tk.W, pady=5)
        
        # Retries
        ttk.Label(tab, text="Попытки (Retries):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.retries_var = tk.StringVar(value="3")
        retries_spinbox = ttk.Spinbox(tab, from_=0, to=10, textvariable=self.retries_var, width=10)
        ...
        # ProxyAPI
        ttk.Label(tab, text="ProxyAPI URL:", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
    ...
    def _create_cache_tab(self, parent: tk.Widget) -> ttk.Frame:
        ...
        ttk.Label(tab, text="TTL топ-запросов (часы):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.ttl_top_var = tk.StringVar(value="24")
        ttl_top_spinbox = ttk.Spinbox(tab, from_=1, to=72, textvariable=self.ttl_top_var, width=10)
        ttl_top_spinbox.pack(anchor=tk.W, pady=5)
        
        # TTL для обычных
        ttk.Label(tab, text="TTL обычных запросов (часы):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.ttl_normal_var = tk.StringVar(value="6")
        ttl_normal_spinbox = ttk.Spinbox(tab, from_=1, to=24, textvariable=self.ttl_normal_var, width=10)
        ttl_normal_spinbox.pack(anchor=tk.W, pady=5)
        
        # TTL для редких
        ttk.Label(tab, text="TTL редких запросов (часы):", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.ttl_rare_var = tk.StringVar(value="1")
        ttl_rare_spinbox = ttk.Spinbox(tab, from_=0, to=12, textvariable=self.ttl_rare_var, width=10)
        ...
        # Redis
        ttk.Label(tab, text="Redis Host:", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        ...
        ttk.Label(tab, text="Redis Port:", font=("Segoe UI", 10)).pack(anchor=tk.W, pady=5)
        self.redis_port_var = tk.StringVar(value="6379")
        redis_port_spinbox = ttk.Spinbox(tab, from_=1, to=65535, textvariable=self.redis_port_var, width=10)
    ...
    def _save_config(self) -> None:
        ...
        messagebox.showinfo("Сохранение", "Конфигурация сохранена в gui_debugger/config.json")
