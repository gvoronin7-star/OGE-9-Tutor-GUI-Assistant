...
class LLMMonitor(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        """Создание виджетов."""
        # Заголовок
        header = ttk.Label(self, text="🤖 МОНИТОРИНГ LLM", font=("Segoe UI", 14, "bold"))
        header.pack(pady=10)
        ...
        self.requests_card = self._create_stat_card(cards_frame, "Запросов сегодня", "0", 0, 0)
        self.avg_time_card = self._create_stat_card(cards_frame, "Ср. время", "0с", 0, 1)
        ...
        refresh_btn = ttk.Button(btn_frame, text="🔄 Обновить", command=self._refresh_status)
        refresh_btn.pack(side=tk.LEFT, padx=5)
        
        test_btn = ttk.Button(btn_frame, text="🧪 Тест соединения", command=self._test_connection)
    ...
    def _refresh_status(self) -> None:
        ...
        self.status_indicator.configure(text="● GigaChat-Max (онлайн)", foreground="#107c10")
        ...
        self.limit_label.configure(text=f"{used} / {limit} запросов ({percentage:.0f}%)")
