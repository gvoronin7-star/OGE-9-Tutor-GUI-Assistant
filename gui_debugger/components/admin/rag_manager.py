...
class RAGManager(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        info_text.insert(tk.END, """Операция замены базы данных:
...
""")
    ...
    def _check_new_base(self) -> None:
        ...
        if missing:
            ...
            self.new_base_info_label.configure(
                text=f"✅ База готова: {chunks_count} чанков",
                foreground="#107c10"
            )
    ...
    def _upload_base(self) -> None:
        ...
        self.operation_status_label.configure(text="Начинаю загрузку...", foreground="#0078d4")
    ...
    def _upload_success(self) -> None:
        ...
        # Логирование
        log_action("rag_base_replaced", {
            "new_path": str(self.new_base_path),
            "success": True
        })
    ...
    def _update_status(self) -> None:
        ...
        # Путь
        self.current_path_label.configure(
            text=f"Путь: {current_base.absolute()}",
            foreground="#303030"
        )
        ...
        try:
            ...
            chunks_count = len(list(chunks_path.glob("*.md"))) if chunks_path.exists() else 0
            metadata_count = len(list(metadata_path.glob("*.json"))) if metadata_path.exists() else 0
            indices_count = len(list(indices_path.glob("*"))) if indices_path.exists() else 0
            ...
            self.current_stats_label.configure(
                text=f"Статистика: Ошибка ({str(e)})",
                foreground="#e81123"
            )
