...
class DatabasePanel(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        """Создание виджетов."""
        # Заголовок
        header = ttk.Label(self, text="📚 DATABASE (RAG_data_base)", font=("Segoe UI", 14, "bold"))
        header.pack(pady=10)
        ...
        search_entry = ttk.Entry(search_frame, textvariable=self.search_query_var, width=40)
        search_entry.pack(side=tk.LEFT, padx=5)
        search_entry.bind("<Return>", lambda e: self._search_database())
        
        search_btn = ttk.Button(search_frame, text="Найти", command=self._search_database)
        search_btn.pack(side=tk.LEFT, padx=5)
        
        clear_btn = ttk.Button(search_frame, text="Очистить", command=self._clear_search)
        ...
        scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.chunks_listbox.yview)
        ...
        try:
            if self.rag_pipeline.existing_store:
                ...
                for i, chunk in enumerate(self.chunks):
                    topic = chunk.get("metadata", {}).get("keywords", ["unknown"])[0] if chunk.get("metadata", {}).get("keywords") else "unknown"
                    chunk_type = chunk.get("type", "unknown")
                    self.chunks_listbox.insert(tk.END, f"[{i:03d}] {topic} ({chunk_type})")
    ...
    def _clear_search(self) -> None:
        ...
        for i, chunk in enumerate(self.chunks):
            topic = chunk.get("metadata", {}).get("keywords", ["unknown"])[0] if chunk.get("metadata", {}).get("keywords") else "unknown"
            self.chunks_listbox.insert(tk.END, f"[{i:03d}] {topic} ({chunk.get('type', 'unknown')})")
