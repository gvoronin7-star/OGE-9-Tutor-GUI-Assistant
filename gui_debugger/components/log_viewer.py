...
class LogViewer(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        # Кнопки
        ttk.Button(top_frame, text="🔄 Обновить", command=self._load_logs).pack(
            side=tk.LEFT, padx=10
        )
        ttk.Button(top_frame, text="🗑️ Очистить", command=self._clear_logs).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(top_frame, text="📤 Экспорт", command=self._export_logs).pack(
            side=tk.LEFT, padx=5
        )
        ...
        self.details_text = scrolledtext.ScrolledText(
            details_frame, wrap=tk.WORD, font=("Consolas", 9), height=8
        )
    ...
    def _on_select(self, event: tk.Event) -> None:
        ...
        if index < len(self.current_logs):
            ...
            if isinstance(log_entry, dict):
                details = json.dumps(
                    log_entry, indent=2, ensure_ascii=False, default=str
                )
            else:
                    ...
                    if file_path.endswith(".json"):
                        ...
                        if self.current_logs:
                            writer = csv.DictWriter(
                                f, fieldnames=self.current_logs[0].keys()
                            )
                            writer.writeheader()
