...
class UserMode(ttk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        if self.on_back_to_selector:
            back_btn = ttk.Button(
                top_bar,
                text="🏠 К выбору режима",
                command=self.on_back_to_selector
            )
    ...
    def _show_topic_study(self) -> None:
        ...
        def on_topic_complete(topic: str):
            ...
            if topic not in self.user_data["progress"]["topics_completed"]:
                ...
                self.user_data["progress"]["topics_studied"] = len(self.user_data["progress"]["topics_completed"])
    ...
    def _show_test_solver(self) -> None:
        ...
        def on_test_complete(topic: str, score: int, total: int):
            ...
            # Сохраняем в user_data
            self.user_data["progress"]["tests_completed"] = self.user_data["progress"].get("tests_completed", 0) + 1
            self.user_data["progress"]["total_questions"] = self.user_data["progress"].get("total_questions", 0) + total
            self.user_data["progress"]["correct_answers"] = self.user_data["progress"].get("correct_answers", 0) + score
