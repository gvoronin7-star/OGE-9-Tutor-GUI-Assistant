...
Ответ (структурированный, с объяснением логики):"""
    ...
    async def initialize(self) -> None:
        ...
        if use_existing_index:
            ...
            try:
                self.existing_store = ExistingVectorStore(
                    Path("RAG_data_base/vector_db")
                )
                loaded = await self.existing_store.load()

                if loaded:
                    logger.info("✓ RAG_data_base загружена (157 чанков ФИПИ)")
                    self.use_existing = True
                else:
                    logger.warning(
                        "Не удалось загрузить RAG_data_base, используем fallback"
                    )
                    self.use_existing = False
        ...
        try:
            ...
            logger.warning(
                f"Не удалось загрузить векторное хранилище: {e}. Будет создано новое."
            )
            self.vector_store = VectorStore(self.indices_dir)
        ...
        try:
            ...
            logger.warning(
                f"Не удалось загрузить полнотекстовый поиск: {e}. Будет создан новый."
            )
            self.text_search = TextSearchEngine(self.indices_dir)
...
""",
        ...
        # Создание метаданных
        metadata = {"topics": list(demo_topics.keys()), "created": "2026-03-05"}
    ...
    async def get_answer(
        self, query: str, user_id: int, use_cache: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger_rag.debug(
                f"Поиск чанков: {len(relevant_chunks)} найдено за {search_duration:.2f}ms"
            )
    ...
    async def _search_chunks(
        ...
        if self.use_existing and self.existing_store:
            try:
                ...
                logger_rag.info(
                    f"RAG_data_base: {len(existing_results)} чанков за {search_duration:.2f}ms"
                )
        ...
        if not results and self.vector_store:
            try:
                ...
                logger_rag.info(
                    f"Faiss fallback: {len(faiss_results)} чанков за {search_duration:.2f}ms"
                )
            except Exception as e:
        ...
        if not results and self.text_search:
            try:
                ...
                logger_rag.info(
                    f"Whoosh fallback: {len(whoosh_results)} чанков за {search_duration:.2f}ms"
                )
            except Exception as e:
    ...
    def _generate_demo_answer(self, query: str, chunks: List[Dict[str, Any]]) -> str:
        """
    ...
    async def generate_test(
        ...
        if not relevant_chunks:
            return {"error": "Не удалось найти информацию по теме", "questions": {}}
    ...
    def _generate_demo_questions(
        ...
        for key, difficulties in demo_questions.items():
            if key in topic_lower:
                ...
                questions_list = difficulties.get(
                    difficulty, difficulties.get("easy", [])
                )
                break
        ...
        if not questions_list:
            ...
            if "человек и общество" in demo_questions:
                questions_list = demo_questions["человек и общество"].get(
                    difficulty, []
                )
