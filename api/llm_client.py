...
from utils.advanced_logger import logger_llm, detailed_logger
...
class LLMClient:
    ...
    async def generate(
        ...
        try:
            ...
            logger_llm.info(f"Ответ получен, длина={len(result)}, время={duration:.2f}ms")
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                async with session.post(
                    ...
                    logger_llm.info(f"Ответ от {model} за {duration:.2f}ms, длина={len(content)}")
    ...
    async def generate_questions(
        ...
        """
        request_start = time.time()
        logger_llm.info(f"Генерация вопросов: тема={topic}, сложность={difficulty}, количество={num_questions}")
...
Ответь только JSON, без дополнительного текста:"""
        ...
        try:
            ...
            try:
                ...
                logger_llm.warning(f"Не удалось распарсить JSON: {e}, ответ: {response[:200]}")
                detailed_logger.log_request(
                    component="llm",
                    action="generate_questions",
                    input_data={"topic": topic, "num_questions": num_questions},
            ...
            logger_llm.info(f"Сгенерировано {len(questions)} вопросов за {duration:.2f}ms")
