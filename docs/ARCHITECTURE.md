...
- `GET /api/topics`, `POST /api/ask`, `POST /api/tests/generate` — для
  ...
  Ответ `POST /api/ask` несёт `is_fallback: bool` — признак, что LLM была
  недоступна и текст пришёл из демо-заглушки, а не от модели (найдено и
  закрыто зональным аудитом хаба 2026-09-08, К-4 отчёта)
